...
import { Check, Dot, Loader2, SearchIcon, X } from "lucide-react";
...
function UserResult({ user, selected, onClick }: UserResultProps) {
  return (
    ...
    >
      ...
      {selected && (
        <Check className="size-6 rounded-2xl bg-astro-green p-1 text-white" />
      )}
    </button>
  );
}
...
function SelectedUserTag({ user, onRemove }: SelectedUserTagProps) {
  return (
    <button className="flex cursor-default items-center gap-2 rounded-full border p-1 hover:bg-muted/50">
      <UserAvatar avatarUrl={user.image} size={24} />
      <p className="font-bold">{user.name}</p>
      <X
        onClick={onRemove}
        className="mx-2 size-5 cursor-pointer text-muted-foreground"
      />
    </button>
  );
}
